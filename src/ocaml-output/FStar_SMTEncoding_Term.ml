                   ...
                   fun e  ->
                     ...
                     | (names0,binders,n0) ->
                         let nm =
                           ...
                           Prims.strcat "__lb" uu____2906  in
...
let rec declToSmt : decl -> Prims.string =
  fun decl  ->
    ...
    | DefPrelude  -> prelude ()
    | Caption c ->
        let uu____2971 = FStar_Options.log_queries ()  in
        if uu____2971
        then
          let uu____2972 =
            FStar_All.pipe_right (FStar_Util.splitlines c)
              (fun uu___91_2974  ->
                 match uu___91_2974 with | [] -> "" | h::t -> h)
             in
          FStar_Util.format1 "\n; %s" uu____2972
        else ""
    | DeclFun (f,argsorts,retsort,c) ->
        let l = FStar_List.map strSort argsorts  in
        let uu____2988 = caption_to_string c  in
        let uu____2989 = strSort retsort  in
        FStar_Util.format4 "%s(declare-fun %s (%s) %s)" uu____2988 f
          (FStar_String.concat " " l) uu____2989
    | DefineFun (f,arg_sorts,retsort,body,c) ->
        let uu____2997 = name_macro_binders arg_sorts  in
        (match uu____2997 with
         | (names,binders) ->
             let body1 =
               let uu____3015 =
                 FStar_List.map (fun x  -> mkFreeV x norng) names  in
               inst uu____3015 body  in
             let uu____3022 = caption_to_string c  in
             let uu____3023 = strSort retsort  in
             let uu____3024 = termToSmt (escape f) body1  in
             FStar_Util.format5 "%s(define-fun %s (%s) %s\n %s)" uu____3022 f
               (FStar_String.concat " " binders) uu____3023 uu____3024)
    | Assume a ->
        let fact_ids_to_string ids =
          FStar_All.pipe_right ids
            (FStar_List.map
               (fun uu___92_3035  ->
                  match uu___92_3035 with
                  | Name n1 ->
           ...
           in
        let fids =
          let uu____3040 = FStar_Options.log_queries ()  in
          if uu____3040
          then
            let uu____3041 =
              let uu____3042 = fact_ids_to_string a.assumption_fact_ids  in
              FStar_String.concat "; " uu____3042  in
            FStar_Util.format1 ";;; Fact-ids: %s\n" uu____3041
          else ""  in
        let n1 = escape a.assumption_name  in
        let uu____3046 = caption_to_string a.assumption_caption  in
      ...
      let uu____3236 =
        let uu____3238 =
          FStar_All.pipe_right constrs
           ...
           in
        FStar_All.pipe_right uu____3238 (FStar_List.map declToSmt)  in
      FStar_All.pipe_right uu____3236 (FStar_String.concat "\n")  in
    let lex_ordering =
...
let mk_Term_uvar : Prims.int -> FStar_Range.range -> term =
  fun i  ->
    fun r  ->
      let uu____3263 =
        let uu____3267 = let uu____3269 = mkInteger' i norng  in [uu____3269]
           in
        ("Tm_uvar", uu____3267)  in
      mkApp uu____3263 r
...
let boxTerm : sort -> term -> term =
  fun sort  ->
    fun t  ->
      ...
      | Ref_sort  -> boxRef t
      | uu____3316 -> raise FStar_Util.Impos
...
let unboxTerm : sort -> term -> term =
  fun sort  ->
    fun t  ->
      ...
      | Ref_sort  -> unboxRef t
      | uu____3323 -> raise FStar_Util.Impos
  ...
  fun t  ->
    ...
    | App
        (Var
         "Prims.b2t",{
                       tm = App
                         (Var "Prims.op_Equality",uu____3331::t1::t2::[]);
                       freevars = uu____3334; rng = uu____3335;_}::[])
        -> mkEq (t1, t2) t.rng
    | App
        (Var
         "Prims.b2t",{
                       tm = App
                         (Var "Prims.op_disEquality",uu____3342::t1::t2::[]);
                       freevars = uu____3345; rng = uu____3346;_}::[])
        -> let uu____3353 = mkEq (t1, t2) norng  in mkNot uu____3353 t.rng
    | App
        (Var
    ...
    | App
        ...
        ->
        let uu____3424 =
          let uu____3427 = unboxBool t1  in
          let uu____3428 = unboxBool t2  in (uu____3427, uu____3428)  in
        mkAnd uu____3424 t.rng
    | App
        (Var
        ...
        mkOr uu____3439 t.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "Prims.op_Negation",t1::[]);
                       freevars = uu____3445; rng = uu____3446;_}::[])
        -> let uu____3453 = unboxBool t1  in mkNot uu____3453 t1.rng
    | App (Var "Prims.b2t",t1::[]) ->
        let uu___93_3456 = unboxBool t1  in
        {
          tm = (uu___93_3456.tm);
          freevars = (uu___93_3456.freevars);
          rng = (t.rng)
        }
    | uu____3459 -> mkApp ("Valid", [t]) t.rng
...
let mk_HasTypeFuel : term -> term -> term -> term =
  fun f  ->
    fun v1  ->
      fun t  ->
        let uu____3488 = FStar_Options.unthrottle_inductives ()  in
        if uu____3488
...
let mk_String_const : Prims.int -> FStar_Range.range -> term =
  fun i  ->
    fun r  ->
      let uu____3552 =
        let uu____3556 = let uu____3558 = mkInteger' i norng  in [uu____3558]
           in
        ("FString_const", uu____3556)  in
      mkApp uu____3552 r
...
let mk_Precedes : term -> term -> FStar_Range.range -> term =
  fun x1  ->
    fun x2  ->
      fun r  ->
        let uu____3569 = mkApp ("Precedes", [x1; x2]) r  in
        FStar_All.pipe_right uu____3569 mk_Valid
...
let rec n_fuel : Prims.int -> term =
  fun n1  ->
    ...
    then mkApp ("ZFuel", []) norng
      ...
      (let uu____3586 =
         let uu____3590 =
           let uu____3592 = n_fuel (n1 - (Prims.parse_int "1"))  in
           [uu____3592]  in
         ("SFuel", uu____3590)  in
       mkApp uu____3586 norng)
    ...
    fun p2  ->
      fun r  ->
        ...
        | (FStar_Pervasives_Native.Some p11,FStar_Pervasives_Native.Some p21)
            ->
            let uu____3615 = mkAnd (p11, p21) r  in
...
let mk_or_l : term Prims.list -> FStar_Range.range -> term =
  fun l  ->
    fun r  ->
      let uu____3660 = mkFalse r  in
      FStar_List.fold_right (fun p1  -> fun p2  -> mkOr (p1, p2) r) l
        uu____3660
  
let mk_haseq : term -> term =
  fun t  ->
    let uu____3666 = mkApp ("Prims.hasEq", [t]) t.rng  in mk_Valid uu____3666
...
let rec print_smt_term : term -> Prims.string =
  fun t  ->
    ...
    | BoundV n1 ->
        let uu____3680 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "(BoundV %s)" uu____3680
    | FreeV fv ->
        FStar_Util.format1 "(FreeV %s)" (FStar_Pervasives_Native.fst fv)
    | App (op,l) ->
        let uu____3688 = print_smt_term_list l  in
        FStar_Util.format2 "(%s %s)" (op_to_string op) uu____3688
    | Labeled (t1,r1,r2) ->
        let uu____3692 = print_smt_term t1  in
        FStar_Util.format2 "(Labeled '%s' %s)" r1 uu____3692
    | LblPos (t1,s) ->
        let uu____3695 = print_smt_term t1  in
        FStar_Util.format2 "(LblPos %s %s)" s uu____3695
    | Quant (qop,l,uu____3698,uu____3699,t1) ->
        let uu____3709 = print_smt_term_list_list l  in
        let uu____3710 = print_smt_term t1  in
        FStar_Util.format3 "(%s %s %s)" (qop_to_string qop) uu____3709
          uu____3710
    | Let (es,body) ->
        let uu____3715 = print_smt_term_list es  in
        let uu____3716 = print_smt_term body  in
        FStar_Util.format2 "(let %s %s)" uu____3715 uu____3716

and print_smt_term_list : term Prims.list -> Prims.string =
  fun l  ->
    let uu____3719 = FStar_List.map print_smt_term l  in
    FStar_All.pipe_right uu____3719 (FStar_String.concat " ")
    ...
    FStar_List.fold_left
      (fun s  ->
         fun l1  ->
           let uu____3729 =
             let uu____3730 =
               let uu____3731 = print_smt_term_list l1  in
               Prims.strcat uu____3731 " ] "  in
             Prims.strcat "; [ " uu____3730  in
           Prims.strcat s uu____3729) "" l
