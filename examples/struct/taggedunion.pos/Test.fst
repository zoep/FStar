...
=
  ...
  if t = 0ul then (
   ...
   P.write x_ptr (UInt8.logxor x y)
    ...
    assert (t == 1ul);
    let z = P.read (TU.field st_tags p "right") in
    let x = FStar.Int.Cast.uint16_to_uint8 z in
    let v = P.struct_create' s_l [(|"x",x|); (|"y",0uy|)] in
...
=
  ...
  if t = 0ul then (
    ...
    assert (P.readable h2 (P.field s_ptr "x"));
