...
(* Applying a function to all elements of a sequence *)
abstract let map
  (#a #b: Type)
  (f: a -> Tot b)
  (s: seq a)
: Tot (seq b)
= MkSeq (List.Tot.map f (MkSeq?.l s))

abstract val map_charac
  (#a #b: Type)
  (f: a -> Tot b)
  (sa: seq a)
  (sb: seq b)
: Lemma
  (ensures
   (length sa == length sb /\
    (forall (i: nat) . i < length sa ==> index sb i == f (index sa i))) <==>
   equal sb (map f sa))
let map_charac #a #b f sa sb =
  List.map_lemma f (MkSeq?.l sa);
  FStar.Classical.forall_intro (List.map_index f (MkSeq?.l sa))

(* Applying a function to all elements of two sequences of the same length *)
abstract let map2
  (#a #b #c: Type)
  (f: a -> b -> Tot c)
  (sa: seq a)
