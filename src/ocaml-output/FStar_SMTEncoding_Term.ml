...
let uu___is_Get_value : decl -> Prims.bool =
  fun projectee  ->
    match projectee with | Get_value _0 -> true | uu____730 -> false
                 ...
                 let pats_str =
                   ...
                   | uu____2832 ->
                       ...
                       FStar_All.pipe_right uu____2835
                         (FStar_String.concat "\n")
                    in
                 let uu____2849 = FStar_Options.smt_solver ()  in
                 (match uu____2849 with
                  | FStar_Options.Z3  ->
                      let uu____2850 =
                        let uu____2852 =
                          let uu____2854 =
                            let uu____2856 = aux1 n2 names1 body  in
                            let uu____2857 =
                              let uu____2859 = weightToSmt wopt  in
                              [uu____2859; pats_str; qid]  in
                            uu____2856 :: uu____2857  in
                          binders1 :: uu____2854  in
                        (qop_to_string qop) :: uu____2852  in
                      FStar_Util.format
                        "(%s (%s)\n (! %s\n %s\n%s\n:qid %s))" uu____2850
                  | FStar_Options.CVC4  ->
                      let uu____2860 =
                        let uu____2862 =
                          let uu____2864 =
                            let uu____2866 = aux1 n2 names1 body  in
                            let uu____2867 =
                              let uu____2869 = weightToSmt wopt  in
                              [uu____2869; pats_str]  in
                            uu____2866 :: uu____2867  in
                          binders1 :: uu____2864  in
                        (qop_to_string qop) :: uu____2862  in
              ...
              FStar_List.fold_left
                (fun uu____2889  ->
                   fun e  ->
                     ...
                     | (names0,binders,n0) ->
                         let nm =
                           let uu____2917 = FStar_Util.string_of_int n0  in
                           Prims.strcat "__lb" uu____2917  in
                         let names01 = (nm, Term_sort) :: names0  in
                         let b =
                           let uu____2925 = aux1 n1 names e  in
                           FStar_Util.format2 "(%s %s)" nm uu____2925  in
                         (names01, (b :: binders),
                           (n0 + (Prims.parse_int "1")))) (names, [], n1) es
               in
            (match uu____2874 with
             | (names1,binders,n2) ->
                 let uu____2943 = aux1 n2 names1 body  in
                 FStar_Util.format2 "(let (%s)\n%s)"
                   (FStar_String.concat " " binders) uu____2943)
      
      and aux depth n1 names t1 =
        let s = aux' depth n1 names t1  in
        if t1.rng <> norng
        then
          let uu____2950 = FStar_Range.string_of_range t1.rng  in
          let uu____2951 = FStar_Range.string_of_use_range t1.rng  in
          FStar_Util.format3 "\n;; def=%s; use=%s\n%s\n" uu____2950
            uu____2951 s
        else s
...
let rec declToSmt : decl -> Prims.string =
  fun decl  ->
    ...
    | DefPrelude  -> prelude ()
    | Caption c ->
        let uu____2982 = FStar_Options.log_queries ()  in
        if uu____2982
        then
          let uu____2983 =
            FStar_All.pipe_right (FStar_Util.splitlines c)
              (fun uu___91_2985  ->
                 match uu___91_2985 with | [] -> "" | h::t -> h)
             in
          FStar_Util.format1 "\n; %s" uu____2983
        else ""
    | DeclFun (f,argsorts,retsort,c) ->
        let l = FStar_List.map strSort argsorts  in
        let uu____2999 = caption_to_string c  in
        let uu____3000 = strSort retsort  in
        FStar_Util.format4 "%s(declare-fun %s (%s) %s)" uu____2999 f
          (FStar_String.concat " " l) uu____3000
    | DefineFun (f,arg_sorts,retsort,body,c) ->
        let uu____3008 = name_macro_binders arg_sorts  in
        (match uu____3008 with
         | (names,binders) ->
             let body1 =
               let uu____3026 =
                 FStar_List.map (fun x  -> mkFreeV x norng) names  in
               inst uu____3026 body  in
             let uu____3033 = caption_to_string c  in
             let uu____3034 = strSort retsort  in
             let uu____3035 = termToSmt (escape f) body1  in
             FStar_Util.format5 "%s(define-fun %s (%s) %s\n %s)" uu____3033 f
               (FStar_String.concat " " binders) uu____3034 uu____3035)
    | Assume a ->
        let fact_ids_to_string ids =
          FStar_All.pipe_right ids
           ...
           in
        let fids =
          let uu____3051 = FStar_Options.log_queries ()  in
          if uu____3051
          then
            let uu____3052 =
              let uu____3053 = fact_ids_to_string a.assumption_fact_ids  in
              FStar_String.concat "; " uu____3053  in
            FStar_Util.format1 ";;; Fact-ids: %s\n" uu____3052
          else ""  in
        let n1 = escape a.assumption_name  in
        let uu____3057 = caption_to_string a.assumption_caption  in
        let uu____3058 = termToSmt n1 a.assumption_term  in
        FStar_Util.format4 "%s%s(assert (! %s\n:named %s))" uu____3057 fids
          uu____3058 n1
    | Get_value (x,uu____3060) -> FStar_Util.format1 "(get-value %s)" x
    | Echo s -> FStar_Util.format1 "(echo \"%s\")" s
    | RetainAssumptions uu____3062 -> ""
    | CheckSat  -> "(check-sat)"
    | GetUnsatCore  ->
        "(echo \"<unsat-core>\")\n(get-unsat-core)\n(echo \"</unsat-core>\")"
    | Push  -> "(push)"
      ...
      let uu____3247 =
        let uu____3249 =
          FStar_All.pipe_right constrs
           ...
           in
        FStar_All.pipe_right uu____3249 (FStar_List.map declToSmt)  in
      FStar_All.pipe_right uu____3247 (FStar_String.concat "\n")  in
    let lex_ordering =
...
let mk_Term_uvar : Prims.int -> FStar_Range.range -> term =
  fun i  ->
    fun r  ->
      let uu____3274 =
        let uu____3278 = let uu____3280 = mkInteger' i norng  in [uu____3280]
           in
        ("Tm_uvar", uu____3278)  in
      mkApp uu____3274 r
...
let boxTerm : sort -> term -> term =
  fun sort  ->
    fun t  ->
      ...
      | Ref_sort  -> boxRef t
      | uu____3327 -> raise FStar_Util.Impos
...
let unboxTerm : sort -> term -> term =
  fun sort  ->
    fun t  ->
      ...
      | Ref_sort  -> unboxRef t
      | uu____3334 -> raise FStar_Util.Impos
  ...
  fun t  ->
    ...
    | App
        (Var
         "Prims.b2t",{
                       tm = App
                         (Var "Prims.op_Equality",uu____3342::t1::t2::[]);
    ...
    | App
        (Var
         "Prims.b2t",{
                       tm = App
                         (Var "Prims.op_disEquality",uu____3353::t1::t2::[]);
                       freevars = uu____3356; rng = uu____3357;_}::[])
        -> let uu____3364 = mkEq (t1, t2) norng  in mkNot uu____3364 t.rng
    | App
        (Var
        ...
        ->
        let uu____3375 =
          let uu____3378 = unboxInt t1  in
          let uu____3379 = unboxInt t2  in (uu____3378, uu____3379)  in
        mkLTE uu____3375 t.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "Prims.op_LessThan",t1::t2::[]);
                       freevars = uu____3382; rng = uu____3383;_}::[])
        ->
        let uu____3390 =
          let uu____3393 = unboxInt t1  in
          let uu____3394 = unboxInt t2  in (uu____3393, uu____3394)  in
        mkLT uu____3390 t.rng
    | App
        (Var
         "Prims.b2t",{
                       tm = App
                         (Var "Prims.op_GreaterThanOrEqual",t1::t2::[]);
                       freevars = uu____3397; rng = uu____3398;_}::[])
        ->
        let uu____3405 =
          let uu____3408 = unboxInt t1  in
          let uu____3409 = unboxInt t2  in (uu____3408, uu____3409)  in
        mkGTE uu____3405 t.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "Prims.op_GreaterThan",t1::t2::[]);
                       freevars = uu____3412; rng = uu____3413;_}::[])
        ->
        let uu____3420 =
          let uu____3423 = unboxInt t1  in
          let uu____3424 = unboxInt t2  in (uu____3423, uu____3424)  in
        mkGT uu____3420 t.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "Prims.op_AmpAmp",t1::t2::[]);
                       freevars = uu____3427; rng = uu____3428;_}::[])
        ->
        let uu____3435 =
          let uu____3438 = unboxBool t1  in
          let uu____3439 = unboxBool t2  in (uu____3438, uu____3439)  in
        mkAnd uu____3435 t.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "Prims.op_BarBar",t1::t2::[]);
                       freevars = uu____3442; rng = uu____3443;_}::[])
        ->
        let uu____3450 =
          let uu____3453 = unboxBool t1  in
          let uu____3454 = unboxBool t2  in (uu____3453, uu____3454)  in
        mkOr uu____3450 t.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "Prims.op_Negation",t1::[]);
                       freevars = uu____3456; rng = uu____3457;_}::[])
        -> let uu____3464 = unboxBool t1  in mkNot uu____3464 t1.rng
    | App (Var "Prims.b2t",t1::[]) ->
        let uu___93_3467 = unboxBool t1  in
        {
          tm = (uu___93_3467.tm);
          freevars = (uu___93_3467.freevars);
          rng = (t.rng)
        }
    | uu____3470 -> mkApp ("Valid", [t]) t.rng
...
let mk_HasTypeFuel : term -> term -> term -> term =
  fun f  ->
    fun v1  ->
      fun t  ->
        let uu____3499 = FStar_Options.unthrottle_inductives ()  in
        if uu____3499
...
let mk_String_const : Prims.int -> FStar_Range.range -> term =
  fun i  ->
    fun r  ->
      let uu____3563 =
        let uu____3567 = let uu____3569 = mkInteger' i norng  in [uu____3569]
           in
        ("FString_const", uu____3567)  in
      mkApp uu____3563 r
...
let mk_Precedes : term -> term -> FStar_Range.range -> term =
  fun x1  ->
    fun x2  ->
      fun r  ->
        let uu____3580 = mkApp ("Precedes", [x1; x2]) r  in
        FStar_All.pipe_right uu____3580 mk_Valid
...
let rec n_fuel : Prims.int -> term =
  fun n1  ->
    ...
    then mkApp ("ZFuel", []) norng
      ...
      (let uu____3597 =
         let uu____3601 =
           let uu____3603 = n_fuel (n1 - (Prims.parse_int "1"))  in
           [uu____3603]  in
         ("SFuel", uu____3601)  in
       mkApp uu____3597 norng)
    ...
    fun p2  ->
      fun r  ->
        ...
        | (FStar_Pervasives_Native.Some p11,FStar_Pervasives_Native.Some p21)
            ->
            let uu____3626 = mkAnd (p11, p21) r  in
            FStar_Pervasives_Native.Some uu____3626
...
let mk_or_l : term Prims.list -> FStar_Range.range -> term =
  fun l  ->
    fun r  ->
      let uu____3671 = mkFalse r  in
      FStar_List.fold_right (fun p1  -> fun p2  -> mkOr (p1, p2) r) l
        uu____3671
  
let mk_haseq : term -> term =
  fun t  ->
    let uu____3677 = mkApp ("Prims.hasEq", [t]) t.rng  in mk_Valid uu____3677
...
let rec print_smt_term : term -> Prims.string =
  fun t  ->
    ...
    | BoundV n1 ->
        let uu____3691 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "(BoundV %s)" uu____3691
    | FreeV fv ->
        FStar_Util.format1 "(FreeV %s)" (FStar_Pervasives_Native.fst fv)
    | App (op,l) ->
        let uu____3699 = print_smt_term_list l  in
        FStar_Util.format2 "(%s %s)" (op_to_string op) uu____3699
    | Labeled (t1,r1,r2) ->
        let uu____3703 = print_smt_term t1  in
        FStar_Util.format2 "(Labeled '%s' %s)" r1 uu____3703
    | LblPos (t1,s) ->
        let uu____3706 = print_smt_term t1  in
        FStar_Util.format2 "(LblPos %s %s)" s uu____3706
    | Quant (qop,l,uu____3709,uu____3710,t1) ->
        let uu____3720 = print_smt_term_list_list l  in
        let uu____3721 = print_smt_term t1  in
        FStar_Util.format3 "(%s %s %s)" (qop_to_string qop) uu____3720
          uu____3721
    | Let (es,body) ->
        let uu____3726 = print_smt_term_list es  in
        let uu____3727 = print_smt_term body  in
        FStar_Util.format2 "(let %s %s)" uu____3726 uu____3727

and print_smt_term_list : term Prims.list -> Prims.string =
  fun l  ->
    let uu____3730 = FStar_List.map print_smt_term l  in
    FStar_All.pipe_right uu____3730 (FStar_String.concat " ")
    ...
    FStar_List.fold_left
      (fun s  ->
         fun l1  ->
           let uu____3740 =
             let uu____3741 =
               let uu____3742 = print_smt_term_list l1  in
               Prims.strcat uu____3742 " ] "  in
             Prims.strcat "; [ " uu____3741  in
           Prims.strcat s uu____3740) "" l
